...
const DetailInfoBtnBox = styled.div`
    width: 22rem;
  @media only screen and (max-width: 1000px) {
    width: 18rem;
  }
...
export function Banner({ bannerInfo, detailSearchUrl, requestUrl }: {
  bannerInfo: IData;
  detailSearchUrl: string;
  requestUrl: string;
}) {
  const bigMatch: PathMatch | null = useMatch(`:menuName/banner/:id`);
  ...
  return (
    ...
    >
      ...
      <AnimatePresence >
        {bigMatch ? (
          <Modal
            layoutId={String(bigMatch?.params.id)}
